    ...
    public class ICalHandler : IHttpHandler
    {
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (this.TryParseCalendar(courseStrings, out calendar))
            {
                ...
                context.Response.Headers["Content-Disposition"] = "attachment; filename=2022_2023_Calendar.ics";
                        ...
                        if (utSection != null && utCourse != null)
                        {
                            ...
                            foreach (CourseSectionTimeSpan timeSpan in utSection.ParsedTime.MeetTimes)
                            {
                                if (utCourse.SemesterDetail == "Fall" || utCourse.SemesterDetail == "Year")
                                {
                                    ...
                                    {
                                        courseEvent.Start = new iCalDateTime(this.ConvertDateTime(new DateTime(2022, 9, 1), timeSpan.Day, timeSpan.Start), "America/Toronto");
                                        courseEvent.End = new iCalDateTime(this.ConvertDateTime(new DateTime(2022, 9, 1), timeSpan.Day, timeSpan.End), "America/Toronto");
                                        courseEvent.RecurrenceRules.Add(new RecurrencePattern(FrequencyType.Weekly)
                                        {
                                            //Count = 13
                                            Until = new DateTime(2022, 12, 20)
                                        });
                                    }
                                    else
                                    {
                                ...
                                if (utCourse.SemesterDetail == "Winter" || utCourse.SemesterDetail == "Year")
                                {
                                    ...
                                    if (utCourse.Faculty == "Engineering")
                                    {
                                        courseEvent.Start = new iCalDateTime(this.ConvertDateTime(new DateTime(2023, 1, 3), timeSpan.Day, timeSpan.Start), "America/Toronto");
                                        courseEvent.End = new iCalDateTime(this.ConvertDateTime(new DateTime(2023, 1, 3), timeSpan.Day, timeSpan.End), "America/Toronto");

                                        courseEvent.RecurrenceRules.Add(new RecurrencePattern(FrequencyType.Weekly)
                                        {
                                            Until = new DateTime(2023, 4, 15)
                                            //Count = 13
                                        });
                                    }
                                    else
